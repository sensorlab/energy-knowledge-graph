...
if __name__ == "__main__":
    ...
    model_path : Path = Path(C.MODEL_PATH).resolve()

    # path to the labels generated by generate_training_data.py
    labels_path : Path = Path(C.LABELS_PATH).resolve()

    # path to the folder to save the predicted appliances
    predicted_appliances_path : Path = Path(C.PREDICTED_APPLIANCES_PATH).resolve()
    ...
    knowledge_graph_endpoint = C.KNOWLEDGE_GRAPH_ENDPOINT
    ...
    training_datsets = C.TRAINING_DATASETS
    # datasets on which to predict appliances
    predict_datasets = C.PREDICT_DATASETS
    ...
    "db-reset" : lambda : reset_database(generated_metadata_path/"residential_metadata.parquet", loadprofiles_path/"merged_loadprofiles.pkl", consumption_data_path/"consumption_data.pkl", datasets, C.POSTGRES_URL),
