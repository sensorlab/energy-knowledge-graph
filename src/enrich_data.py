...
def get_country_GDP(country: str, year: int) -> float | None:
...
def get_average_wages(country: str, year: int) -> float | None:
    ...
    wages_processed = wages[wages["TIME"] == year].drop(
        columns=["TIME", "INDICATOR", "SUBJECT", "MEASURE", "FREQUENCY", "Flag Codes"]).set_index("LOCATION")
...
def get_education_level(country: str, year: int) -> float:
    ...
    df = df[df["TIME"] == year].drop(columns=["TIME", "Flag Codes", "MEASURE", "FREQUENCY", "INDICATOR"]).set_index(
        "LOCATION")
    ...
    try:
        education_lvls = [df.loc[country_code].values[0][1], df.loc[country_code].values[2][1],
                          df.loc[country_code].values[1][1]]
...
def get_gas_price(country: str, date: date) -> float:
    ...
    df = df.drop(
        columns=["DATAFLOW", "LAST UPDATE", "freq", "product", "nrg_cons", "unit", "tax", "currency", "OBS_FLAG"])
